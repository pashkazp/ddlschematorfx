...
### 3.1.
Управління підключеннями до баз даних
...
- **Використання DBMS_METADATA.GET_DDL:** Використання стандартного Oracle пакету `DBMS_METADATA` для отримання DDL об'єктів.
Врахування параметрів `DBMS_METADATA` для отримання повного DDL, включаючи специфічні для Oracle 21c можливості.
...
- **Використання наданої схеми з файлової системи:** Можливість завантаження та використання схеми з двох типів файлових джерел:
    - **Завантаження зі структурованого DDL-сховища:** Можливість завантаження схеми з директорії, що містить окремі DDL-файли для кожного об'єкта, згруповані за типом об'єкта (наприклад, у форматі, в якому додаток зберігає витягнуті схеми).
    - **Завантаження з монолітного DDL-файлу:** Можливість завантаження схеми з єдиного SQL DDL-файлу, що містить визначення для багатьох об'єктів схеми.
- **Парсинг DDL для побудови внутрішнього представлення схеми:**
    - **Для структурованого DDL-сховища:** Зчитування окремих DDL-файлів та їх асоціація з відповідними об'єктами схеми (тип, власник, ім'я) для побудови внутрішнього представлення.
    - **Для монолітного DDL-файлу:** Розбір (парсинг) файлу з метою виокремлення DDL-визначень для окремих об'єктів схеми (наприклад, таблиць, процедур, функцій тощо) та їх атрибутів (тип, власник, ім'я). Після розбору, ці окремі DDL-визначення використовуються для побудови внутрішнього представлення схеми. (Опціонально: після розбору монолітного файлу, його вміст може бути збережений у форматі структурованого DDL-сховища для уніфікації подальшої обробки).
...
### 3.4.
Генерація скриптів синхронізації
...
- **Продуктивність:** Забезпечення прийнятної продуктивності при роботі з великими схемами.
Використання асинхронних операцій для тривалих процесів (витягнення, порівняння, застосування скриптів) для забезпечення чутливості інтерфейсу.
...
- **Presentation Layer (JavaFX):** Відповідає за графічний інтерфейс користувача.
Взаємодіє з Business Logic Layer для отримання даних та ініціації операцій.
Використовує FXML для опису інтерфейсу та контролери для обробки подій.
...
- **Persistence Layer (JDBC, Jasypt, JGit, java-gitlab-api):** Відповідає за взаємодію із зовнішніми джерелами даних: база даних Oracle (через JDBC), файлова система (збереження конфігурацій, DDL файлів), GitLab (через JGit та GitLab API).
Відповідає за отримання DDL через DBMS_METADATA, збереження/завантаження конфігурацій (з шифруванням), виконання SQL-скриптів, взаємодію з Git-репозиторіумом та GitLab API.
...
""";
    ...
    public static void main(String[] args) {
        // Приклад, як можна отримати текст завдання (не є частиною самого ТЗ)
         String tsText = TechnicalSpecification.document;
